...
@Component({
    selector: 'app-view-services-pricing',
    templateUrl: './view-services-pricing.component.html',
    styleUrl: './view-services-pricing.component.scss',
})
export class ViewServicesPricingComponent implements OnInit {
        ...
        columns: [
            ...
            // },
            {
                header: this._transloco.translate(
                    'servicesPricing.view.table.typeOfService'
                ),
                field: 'typeOfService',
                type: 'text',
                sortable: false,
    ...
    ngOnInit(): void {
        this.getPermissions();
        this.initSearchForm();
        this.getAllServicesPricingList();
    }
    ...
    getAllServicesPricingList(): void {
        this.tableConfig.loading = true;
        this._basicService
            .getAll(ENDPOINT.SERVICES_PRICING, this.filterCriteria)
    ...
    navigateToAddNewService(): void {
        this._router.navigate([`Services-Pricing/add-edit`]);
    }

    navigateToDeletedService(): void {
        this._router.navigate([`Services-Pricing/deleted`]);
    }
    ...
    search(): void {
        // this.filterCriteria.search = this.searchForm.controls['search'].value;
        this.filterCriteria.readDto.typeOfService = this.searchForm.controls['typeOfService'].value;
    ...
    askDeleteService(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            ...
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_service',
    ...
    lock(item): void {
        this._basicService
            .lock(ENDPOINT.SERVICES_PRICING, item.id)
            .pipe(takeUntilDestroyed(this._destroyRef))
        ...
        switch (ev.key) {
            ...
            case 'EDIT':
                this._router.navigate([
                    `/Services-Pricing/add-edit/${ev.item.id}`,
                ]);
                break;
            case 'DELETE':
    ...
    onDeleteService(id: any): void {
        this._basicService
            .delete(ENDPOINT.SERVICES_PRICING, id)
            .subscribe((res: any) => {
                this.deletedService();
            });
    }
