...
@Component({
    ...
    styleUrl: './view-workers.component.scss',
})
export class ViewWorkersComponent implements OnInit{
    ...
    tableConfig = {
        columns: [
            ...
            {
                header: this._transloco.translate('workers.view.table.name'),
                field: 'name',
                type: 'text',
                sortable: false,
                style: 'text-center',
            ...
            {
                header: this._transloco.translate('workers.view.table.age'),
                field: 'age',
                type: 'text',
                sortable: false,
                style: 'text-center',
            ...
            {
                header: this._transloco.translate('workers.view.table.job'),
                field: 'jobName',
                type: 'text',
                sortable: false,
                style: 'text-center',
            ...
            {
                header: this._transloco.translate(
                    'workers.view.table.nationality'
                ),
    ...
    searchtOptions = [
        {
            label: this._transloco.translate(
                'workers.view.filter.place_number'
            ),
            value: 'place_number',
            icon: 'heroicons_outline:user',
        },
        {
            label: this._transloco.translate('workers.view.filter.worker_name'),
            value: 'worker_name',
            icon: 'heroicons_outline:user',
        },
    ];
    ...
    onselectedComponent(e) {
        console.log(e.value.value);
        this.selectedComponent = e.value.value;
    }

    getAllWorkersList(): void {
        this.tableConfig.loading = true;
        this._basicService
            .getAll(ENDPOINT.WORKER, this.filterCriteria)
    ...
    navigateToAddNewWorkers(): void {
        this._router.navigate([`Workers/add-edit`]);
    }

    navigateToDeletedWorkers(): void {
        this._router.navigate([`Workers/deleted`]);
    }
    ...
    search(): void {
        ...
        // this.filterCriteria.readDto.residenceNumber = this.searchForm.controls['residenceNumber'].value;
    ...
    askDeleteWorker(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_worker',
                description: 'dynamicDialog.delete_worker_description',
    ...
    deletedWorker(): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            ...
            data: {
                ...
                description: 'dynamicDialog.delete_worker',
                cancelText: 'dynamicDialog.back',
                image: 'assets/images/ui/Inbox_cleanup.gif',
            },
        });
    ...
    onActionClick(ev: { key: string; item: any }): void {
        switch (ev.key) {
            case 'LOCK':
                ...
                break;
            case 'EDIT':
                this._router.navigate([`/Workers/add-edit/${ev.item.id}`]);
                break;
            case 'DELETE':
                this.askDeleteWorker(ev.item.id);
    ...
    onDeleteWorker(id: any): void {
        this._basicService
            .delete(ENDPOINT.WORKER, id)
            .subscribe((res: any) => {
                this.deletedWorker();
            });
    }
