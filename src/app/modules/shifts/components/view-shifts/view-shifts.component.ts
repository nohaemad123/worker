...
export class ViewShiftsComponent implements OnInit {
    ...
    tableConfig = {
        columns: [
            ...
            {
                header: this._transloco.translate('shifts.view.table.name'),
                field: 'name',
                type: 'text',
                sortable: false,
                style: 'text-center',
            ...
            {
                header: this._transloco.translate('shifts.view.table.time'),
                field: 'time',
                type: 'text',
                sortable: false,
                style: 'text-center',
    ...
    navigateToAddNewShift(): void {
        this._router.navigate([`Shifts/add-edit`]);
    }

    navigateToDeletedShift(): void {
        this._router.navigate([`Shifts/deleted`]);
    }
    ...
    askDeleteShift(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_shift',
                description: 'dynamicDialog.delete_shift_description',
    ...
    deletedShifts(): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            ...
            data: {
                ...
                description: 'dynamicDialog.delete_shift',
                cancelText: 'dynamicDialog.back',
                image: 'assets/images/ui/Inbox_cleanup.gif',
            },
        });
    ...
    onActionClick(ev: { key: string; item: any }): void {
        switch (ev.key) {
            case 'LOCK':
                ...
                break;
            case 'EDIT':
                this._router.navigate([`/Shifts/add-edit/${ev.item.id}`]);
                break;
            case 'DELETE':
                this.askDeleteShift(ev.item.id);
    ...
    onDeleteShift(id: any): void {
        this._basicService
            .delete(ENDPOINT.SHIFTS, id)
            .subscribe((res: any) => {
                this.deletedShifts()
            });
    }
