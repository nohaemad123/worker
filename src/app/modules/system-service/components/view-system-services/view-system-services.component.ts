...
@Component({
    selector: 'app-view-system-services',
    templateUrl: './view-system-services.component.html',
    styleUrl: './view-system-services.component.scss'
})

export class ViewSystemServicesComponent {
    ...
    tableConfig = {
        columns: [
            ...
            // {
            //     header: this._transloco.translate('companyBranches.view.table.code'),
            //     field: 'code',
            //     type: 'number',
            //     sortable: false,
            //     style: 'text-center',
            ...
            // },
            {
                header: this._transloco.translate(
                    'systemServices.view.table.name'
                ),
                field: 'name',
                type: 'text',
                sortable: false,
    ...
    navigateToAddNesSystemService(): void {
        this._router.navigate([`Services-Systems/add-edit`]);
    }
    ...
    initSearchForm(): void {
        this.searchForm = this._formBuilder.group({ name: [null] });
    }

    askDeleteCompanySystemServiceList(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_system_service',
                description: 'dynamicDialog.delete_system_service_description',
    ...
    deletedSystemService(): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.Congratulations',
                description: 'dynamicDialog.delete_system_service',
                cancelText: 'dynamicDialog.back',
                image: 'assets/images/ui/Inbox_cleanup.gif',
            },
        });
    ...
    lock(item): void {
        this._basicService
            .lock(ENDPOINT.SERVICES_SYSTEMS, item.id)
            .pipe(takeUntilDestroyed(this._destroyRef))
        ...
        switch (ev.key) {
            ...
            case 'EDIT':
                this._router.navigate([
                    `/Services-Systems/add-edit/${ev.item.id}`,
                ]);
                break;
            case 'DELETE':
                this.askDeleteCompanySystemServiceList(ev.item.id);
    ...
    onDeleteSystemService(id: any): void {
        this._basicService
            .delete(ENDPOINT.SERVICES_SYSTEMS, id)
            .subscribe((res: any) => {
                this.deletedSystemService();
            });
    }
}
