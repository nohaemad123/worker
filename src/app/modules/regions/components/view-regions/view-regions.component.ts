...
@Component({
    ...
    styleUrl: './view-regions.component.scss'
})
export class ViewRegionsComponent implements OnInit {
    ...
    tableConfig = {
        columns: [
            ...
            {
                header: this._transloco.translate('regions.view.table.name'),
                field: 'name',
                type: 'text',
                sortable: false,
                style: 'text-center',
    ...
    getAllRegionsList(): void {
        this.tableConfig.loading = true;
        this._basicService
            .getAll(ENDPOINT.REGION, this.filterCriteria)
    ...
    navigateToAddNewRegion(): void {
        this._router.navigate([`Regions/add-edit`]);
    }

    navigateToDeletedRegion(): void {
        this._router.navigate([`Regions/deleted`]);
    }
    ...
    search(): void {
        ...
        //     .value ? this.searchForm.controls['code'].value
        //     : null;
        this.getAllRegionsList();
    }
    ...
    askDeleteRegion(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_region',
                description: 'dynamicDialog.delete_region_description',
    ...
    deletedRegion(): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            ...
            data: {
                ...
                description: 'dynamicDialog.delete_region',
                cancelText: 'dynamicDialog.back',
                image: 'assets/images/ui/Inbox_cleanup.gif',
            },
        });
    ...
    onActionClick(ev: { key: string; item: any }): void {
        switch (ev.key) {
            ...
            case 'LOCK':
                ...
                break;
            case 'EDIT':
                this._router.navigate([`/Regions/add-edit/${ev.item.id}`]);
                break;
            case 'DELETE':
                this.askDeleteRegion(ev.item.id);
