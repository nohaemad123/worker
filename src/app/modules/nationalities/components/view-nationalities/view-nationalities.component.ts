...
@Component({
    selector: 'app-view-nationalities',
    templateUrl: './view-nationalities.component.html',
    styleUrl: './view-nationalities.component.scss'
})
export class ViewNationalitiesComponent implements OnInit {
    ...
    tableConfig = {
        columns: [
            ...
            // {
            //     header: this._transloco.translate('nationalities.view.table.code'),
            //     field: 'code',
            //     type: 'number',
            //     sortable: false,
            //     style: 'text-center',
            ...
            // },
            {
                header: this._transloco.translate('nationalities.view.table.nameAr'),
                field: 'name',
                type: 'text',
                sortable: false,
                style: 'text-center',
    ...
    navigateToAddNewNationality(): void {
        this._router.navigate([`Nationalities/add-edit`]);
    }

    navigateToDeletedNationality(): void {
        this._router.navigate([`Nationalities/deleted`]);
    }
    ...
    search(): void {
        ...
        // this.filterCriteria.readDto.nationalityCode = this.searchForm.controls['nationalityCode'].value;
    ...
    askDeleteNationality(id: any): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.ask_delete_nationality',
                description: 'dynamicDialog.delete_nationality_description',
    ...
    deletedNationality(): void {
        this.ref = this._dialogService.open(DynamicDialogComponent, {
            width: '30vw',
            height: 'auto',
            data: {
                title: 'dynamicDialog.Congratulations',
                description: 'dynamicDialog.delete_nationality',
                cancelText: 'dynamicDialog.back',
                image: 'assets/images/ui/Inbox_cleanup.gif',
            },
        });
    ...
    lock(item): void {
        this._basicService
            .lock(ENDPOINT.NATIONALITY, item.id)
            .pipe(takeUntilDestroyed(this._destroyRef))
    ...
    onActionClick(ev: { key: string; item: any }): void {
        switch (ev.key) {
            case 'LOCK':
                ...
                break;
            case 'EDIT':
                this._router.navigate([`/Nationalities/add-edit/${ev.item.id}`]);
                break;
            case 'DELETE':
                this.askDeleteNationality(ev.item.id);
    ...
    onDeleteNationality(id: any): void {
        this._basicService
            .delete(ENDPOINT.NATIONALITY, id)
            .subscribe((res: any) => {
                this.deletedNationality()
            });
    }
