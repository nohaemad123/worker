...
import { NavigationPramesService } from '@data/services/navigation-prames.service';
...
import { defaultNavigation } from '@shared/layout/navigation/navigation';
import {
    delay,
    filter,
    merge,
...
export class FuseVerticalNavigationComponent
    ...
    constructor(
        ...
        private _NavigationPramesService: NavigationPramesService
    ) {
    ...
    ngOnInit(): void {
        ...
        this._ActivatedRoute.paramMap.subscribe((params) => {
            // tenant = params.get('tenantId');
            tenant = "test";
        });
        this.handlePermissions(tenant);
    ...
    handlePermissions(tenant): void {
        let permissions = JSON.parse(localStorage.getItem('permissions'));
        this.navigation =
            this._NavigationPramesService.defaultNavigation(tenant);
        this.navigation = this.navigation.map((item: any) => {
            return {
                ...item,
                isActive: this.checkIfPermissionIsActive(item, permissions),
            };
        }).filter(item => item.isActive);
    }

    checkIfPermissionIsActive(item, permissions): boolean {
        let hasPermission = false;
        permissions.forEach((permission) => {
            // console.log('permission', permissionsViewAction);
            if (item.id === permission.pageKey) {
                hasPermission = permission.permissionActions?.filter(perm => perm.permissionAction == 'تصفح')[0].havePermission;
            }
        });
